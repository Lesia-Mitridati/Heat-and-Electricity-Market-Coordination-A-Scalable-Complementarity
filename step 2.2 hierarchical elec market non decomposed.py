...
class hierarchical_elec_scenario_0:
    ...
    def _load_data(self,s0,W_max):
        ...
        # scenarios
        self.data.elec_load = {t:elec_load_scenario_kmeans_0[scenario_dic_kmeans_0[s0][0],t] for t in time}  #SCENARIOS REALIZATIONS!!!!!!!!!!!!!!!!!            
        self.data.wind_scenario = {(w,t):wind_scenario_kmeans_0[scenario_dic_kmeans_0[s0][1],w,t] for w in wind for t in time} #SCENARIOS REALIZATIONS!!!!!!!!!!!!!!!!!
        self.data.alpha = {(g,t):alpha_scenario_kmeans_0[scenario_dic_kmeans_0[s0][2],g,t] for g in CHP+gen+wind for t in time} #SCENARIOS REALIZATIONS!!!!!!!!!!!!!!!!!
...
spot_price_hierarchical_scenario_0 = {}
spot_price_hierarchical_average_0 = {}

for W_max in W_range:   
    
    for s0 in scenario_kmeans_0:
        
        dispatch = hierarchical_elec_scenario_0(s0,W_max)
        dispatch.model.params.OutputFlag = 0
        dispatch.optimize()
 
        for t in time:

            spot_price_hierarchical_scenario_0[W_max,s0,t] = dispatch.constraints.elec_balance[t].Pi

for W_max in W_range:   
           
        spot_price_hierarchical_average_0[W_max,t] = sum(spot_price_hierarchical_scenario_0[W_max,s,t] for s in scenario_kmeans_0)/S_all_kmeans_0
